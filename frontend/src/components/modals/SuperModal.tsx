            ...
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="flex-1 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm text-muted-foreground">
                  ...
                  {type === 'filme' && (midia as Filme).duracao && (<div className="flex items-center gap-1"><Clock className="h-4 w-4" /><span className="font-semibold orbe-text-secondary mr-1">Duração:</span> {formatDuration((midia as Filme).duracao)}</div>)}
                  {type === 'filme' && (midia as Filme).diretor && (<div className="flex items-center gap-1 col-span-2"><span className="font-semibold orbe-text-secondary">Direção:</span> {(midia as Filme).diretor}</div>)}
                  ...
                  {type === 'serie' && (midia as Serie).criadores && (midia as Serie).criadores.length > 0 && (<div className="flex items-center gap-1 col-span-2"><span className="font-semibold orbe-text-secondary">Criador(es):</span> {(midia as Serie).criadores.map(c => c.nome).join(', ')}</div>)}
