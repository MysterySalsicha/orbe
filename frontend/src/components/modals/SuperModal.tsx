            ...
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="flex-1 space-y-4">
                ...
                <div className="flex items-center gap-4 text-sm text-muted-foreground flex-wrap">
                  <div className="flex items-center gap-1"><Calendar className="h-4 w-4" />Lançamento: {midia.data_lancamento_curada || midia.data_lancamento_api ? format(parseISO(midia.data_lancamento_curada || midia.data_lancamento_api), 'dd/MM/yyyy', { locale: ptBR }) : 'N/A'}</div>
                  ...
                  {type === 'filme' && (midia as Filme).duracao && (<div className="flex items-center gap-1"><Clock className="h-4 w-4" /><span className="font-semibold orbe-text-secondary mr-1">Duração:</span> {(midia as Filme).duracao} min</div>)}
                  {type === 'filme' && (midia as Filme).diretor && (<div className="flex items-center gap-1"><span className="font-semibold orbe-text-secondary">Direção:</span> {(midia as Filme).diretor}</div>)}
                  {type === 'serie' && (midia as Serie).numero_temporadas && (<div className="flex items-center gap-1"><Tv className="h-4 w-4" /><span className="font-semibold orbe-text-secondary mr-1">Temporadas:</span> {(midia as Serie).numero_temporadas}</div>)}
                  {type === 'serie' && (midia as Serie).numero_episodios && (<div className="flex items-center gap-1"><span className="font-semibold orbe-text-secondary">Episódios:</span> {(midia as Serie).numero_episodios}</div>)}
                  {type === 'serie' && (midia as Serie).criadores && (midia as Serie).criadores.length > 0 && (<div className="flex items-center gap-1"><span className="font-semibold orbe-text-secondary">Criador(es):</span> {(midia as Serie).criadores.map(c => c.nome).join(', ')}</div>)}
                  ...
                  {type === 'jogo' && (midia as Jogo).desenvolvedores && (midia as Jogo).desenvolvedores.length > 0 && (<div className="flex items-center gap-1"><span className="font-semibold orbe-text-secondary">Desenvolvedor(es):</span> {(midia as Jogo).desenvolvedores.join(', ')}</div>)}
                  {type === 'jogo' && (midia as Jogo).publicadoras && (midia as Jogo).publicadoras.length > 0 && (<div className="flex items-center gap-1"><span className="font-semibold orbe-text-secondary">Publicador(es):</span> {(midia as Jogo).publicadoras.join(', ')}</div>)}
                  {type === 'jogo' && (midia as Jogo).plataformas_jogo && (midia as Jogo).plataformas_jogo.length > 0 && (<div className="flex items-center gap-1"><Gamepad2 className="h-4 w-4" /><span className="font-semibold orbe-text-secondary mr-1">Plataformas:</span> {(midia as Jogo).plataformas_jogo.map(p => p.nome).join(', ')}</div>)}
                </div>
