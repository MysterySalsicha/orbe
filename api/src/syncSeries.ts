...
export const syncSeries = async (year: number, monthToSync: number) => {
  logger.info(`Iniciando sincronização de séries para o ano ${year}, mês ${monthToSync}...`);
  let page = 1;
  let totalSeriesSyncedThisMonth = 0;
  let hasMorePages = true;

  logger.info(`Buscando séries para ${monthToSync}/${year}...`);
  ...
  while (hasMorePages) {
    try {
      const discoverResponse = await tmdbApi.get('/discover/tv', {
        params: {
          ...
          'first_air_date.gte': `${year}-${String(monthToSync).padStart(2, '0')}-01`,
          'first_air_date.lte': `${year}-${String(monthToSync).padStart(2, '0')}-31`,
          page: page,
        },
      });
      ...
      logger.info(`Processando ${seriesToSync.length} séries do mês ${monthToSync} da página ${page}/${discoverResponse.data.total_pages || 'desconhecida'}`);
      ...
      for (const basicSerie of seriesToSync) {
          try {
            ...
            logger.info(`  Série [${serie.id}] "${serie.name}" (Lançamento: ${serie.first_air_date}) (${++serieIndex}/${seriesToSync.length}) sincronizada.`);
            totalSeriesSyncedThisMonth++;
          } catch (detailError: any) {
            logger.error(`  (${serieIndex + 1}/${seriesToSync.length}) Erro ao buscar detalhes para a série ID ${basicSerie.id}: ${detailError.message}`);
          }
    ...
    if (totalSeriesSyncedThisMonth > 0) {
        logger.info(`${totalSeriesSyncedThisMonth} séries de ${monthToSync}/${year} sincronizadas com sucesso.`);
    } else {
        logger.info(`Nenhuma série nova encontrada para ${monthToSync}/${year}.`);
    }
  logger.info(`Total de ${totalSeriesSyncedThisMonth} séries sincronizadas para o ano ${year}, mês ${monthToSync}.`);
};